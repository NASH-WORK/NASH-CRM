...
class FCurl {
	...
	public static function header($url) {
		header('location:http://'.$_SERVER['HTTP_HOST'].'/crm/'.$url);
		exit();
	}
	...
	public static function get($url, $params = array(), $opt = array()) {
		if ($url == 'user/getEventListByGroupId') $opt['ignoreHeader'] = true;

	    $url = 'http://'.$_SERVER['HTTP_HOST'].'/app/?r='.$url;
